...
class Bubble(private val service: AccessibilityService, private val onStop: () -> Unit, private val onCancel: () -> Unit) {
    ...
    private fun show(text: String, newMode: WaveformModel.Mode, stop: Boolean, cancel: Boolean) {
        ...
        mode = newMode
        modelLabel.text = SettingsStore.current.summary.uppercase()
        status.text = text
    ...
    private fun build(): LinearLayout {
        ...
        modelLabel = label(ASH, 11f).apply {
            letterSpacing = 0.12f
        ...
        return LinearLayout(service).apply {
            ...
            addView(modelLabel, LinearLayout.LayoutParams(LinearLayout.LayoutParams.MATCH_PARENT, LinearLayout.LayoutParams.WRAP_CONTENT).apply {
                setMargins(dp(20), dp(14), dp(20), 0)
            })
            addView(wave, LinearLayout.LayoutParams(LinearLayout.LayoutParams.MATCH_PARENT, dp(64)).apply {
                setMargins(dp(20), dp(10), dp(20), 0)
            })
    ...
    companion object {
        ...
        private val ASH = Color.parseColor("#8C8C8C")
    }
}
